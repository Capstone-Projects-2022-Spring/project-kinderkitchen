  ...
  const signInHandler = () => {
    signInWithEmailAndPassword(auth, email, password)
      ...
      .catch((error) => {
        ...
        if (errorCode === 'auth/user-not-found'){alert("Oops! Could not find user with\n Email: " + email)}
        if (errorCode === 'auth/wrong-password'){alert("Incorrect password. \nPlease try again.")}
        if (errorCode === 'auth/missing-email'){alert("Missing Email Parameter")}
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.btnContainer}>
        ...
        {/*Login Button*/}
        <TouchableOpacity style={styles.userBtn} onPress={() => signInHandler()}>
          <Text style={styles.btnTxt}> Login </Text>
        </TouchableOpacity>

        {/*SignUp Button*/}
        <TouchableOpacity style={styles.userBtn} onPress={() => navigation.navigate("Sign Up")}>
